...
import { getHuntsearchers } from 'apiSdk/huntsearchers';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['user-that-searches-a-flat'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<HuntsearcherInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/huntsearchers` : null),
    () => getHuntsearchers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Huntsearcher',
      path: isTenantUser ? `/huntsearchers/view/${data?.[0]?.id}` : '/huntsearchers',
      entity: 'huntsearcher',
      service: AccessServiceEnum.PROJECT,
      icon: FiSearch,
    },
    { name: 'Apartment', path: '/apartments', entity: 'apartment', service: AccessServiceEnum.PROJECT, icon: FiHome },
    { name: 'Platform', path: '/platforms', entity: 'platform', service: AccessServiceEnum.PROJECT, icon: FiGlobe },
